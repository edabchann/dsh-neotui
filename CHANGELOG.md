...
- 欢迎页改版：40×19 半块渲染的 DeepSeek 吉祥物 logo（真实色 255 色调色板 + 逐格上下半色，数据内嵌 `src/logo.js`，去背景、无残块）；品牌行重绘为间距大写 `DEEPSEEK HARNESS` / `DSH NEOTUI`（保留版本号与点击检查更新）；模式选择从内联 4 列表收敛为底部提示「模式: <当前> · F9 打开模式选择（支持自定义）」——F9 或点击进入模式选择 buffer，与 WebUI 鼓励自定义预设的方向一致。
