...
- 欢迎页改版：40×19 半块渲染的 DeepSeek 吉祥物 logo（真实色 255 色调色板 + 逐格上下半色，数据内嵌 `src/logo.js`，去背景、无残块）；品牌行用 **半块像素字** 绘制（`DEEPSEEK` / `DSH NEOTUI` 两行大字标，4×6 像素字模，版本与更新状态右对齐、可点击检查更新），**每约 4 秒一道斜向高光扫过字标**（约 0.7s 扫程，纯 TUI 渲染并随主题语义色）；模式选择从内联 4 列表收敛为底部提示「模式: <当前> · F9 打开模式选择（支持自定义）」——F9 或点击进入模式选择 buffer，与 WebUI 鼓励自定义预设的方向一致；矮视口（<29 行或 <60 列）回退为普通文本品牌行。
