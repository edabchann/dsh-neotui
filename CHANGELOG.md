...
- 欢迎页改版：40×19 半块渲染的 DeepSeek 吉祥物 logo（真实色 255 色调色板 + 逐格上下半色，数据内嵌 `src/logo.js`，去背景、无残块）；品牌行用 **半块像素字** 绘制（`DEEPSEEK` / `DSH NEOTUI` 两行大字标，4×6 像素字模），**每约 3.5 秒一道斜向高光扫过字标**（0.85s 扫程，纯 TUI 渲染并随主题语义色）；**logo 三态** `Ctrl+R` buffer：预设（内置）/ 自定义（Yazi 风格文件选择器单选 `{palette, grid}` 40×19 JSON）/ 关闭（仅标题），持久化到配置文件；版本更新提示到底部（仅「可更新/失败/检查中」时显示，可点击检查，**最新则不提示**）；模式选择从内联 4 列表收敛为底部提示「模式: <当前> · F9 打开模式选择（支持自定义）」——与 WebUI 鼓励自定义预设的方向一致；矮视口（<29 行或 <60 列）回退为普通文本品牌行。；品牌字标注：`DSH NEOTUI` 底色改为深灰保证白色高光可见（此前浅灰吞掉高光，扫过下半即消失）；内置 logo 数据重新生成（裁剪掉原图底部的黑条黑边）
