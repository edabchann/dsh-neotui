...
test("logo picker: Ctrl+R buffer switches preset / custom file / none and persists", () => {
  ...
  assert.ok(app.overlay?.constructor?.name === "UploadPicker", "custom opens the shared Ctrl+O / workspace picker");
  assert.equal(app.overlay.single, true, "single-select mode for the logo file");
  app.overlay.changePath(dirname(file), basename(file));
  app.overlay.onKey({ type: "key", name: "enter" });
